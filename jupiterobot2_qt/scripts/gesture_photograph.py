...
class MediaPipe_Pose:
    ...
    def show_captured_image(self, img):
        # 只更新缓存和标志位，不在子线程中做任何 cv2 操作
        self.show_img_cache = img.copy()
        self.show_img_flag = True

    def handle_photo_taken(self, img):
        # 在单独的线程中保存照片和播放声音
        threading.Thread(target=self.save_and_play_sound, args=(img,)).start()

    def save_and_play_sound(self, img):
        now = datetime.now()
        ...
        try:
            ...
            if self.FLAG_TAKE_PHOTO == True:
                ...
                if self.time_count > 60:
                    ...
                    time_str = now.strftime("%Y%m%d_%H%M%S")
                    img_name = f"/home/mustar/.ros/image_{time_str}.png"
                    cv2.imwrite(img_name, self.out_image)

                    self.handle_photo_taken(self.out_image.copy())

                    # 新开线程显示图片
                    thread = threading.Thread(target=self.show_captured_image, args=(self.out_image.copy(),))
                    thread.start()
            ...
            if self.show_img_flag and self.show_img_cache is not None:
                cv2.imshow("Captured Photo", self.show_img_cache)
                key = cv2.waitKey(2000)  # 显示2秒
                cv2.destroyWindow("Captured Photo")
                self.show_img_flag = False  # 重置标志
